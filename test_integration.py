#!/usr/bin/env python
"""
Integration test to verify CmdrData-OpenAI SDK functionality
"""
...
# Test basic import
try:
    from cmdrdata_openai import TrackedOpenAI, set_customer_context, customer_context
    from cmdrdata_openai.proxy import OPENAI_TRACK_METHODS
    print("[OK] Successfully imported CmdrData-OpenAI")
...
try:
    from cmdrdata_openai.version_compat import check_compatibility, get_compatibility_info
    compat_info = get_compatibility_info()
    print(f"[OK] OpenAI version: {compat_info.get('openai', {}).get('installed', 'Not found')}")
    if check_compatibility():
        print("[OK] OpenAI version is compatible")
    else:
        print("[WARNING] OpenAI version may have compatibility issues")
...
try:
    # Mock the OpenAI client and the API key validation to avoid needing real API keys
    with patch('cmdrdata_openai.client.OpenAI') as MockOpenAI, \
         patch('cmdrdata_openai.security.APIKeyManager.validate_api_key') as MockValidate:
        ...
        client = TrackedOpenAI(
            ...
            tracker_key="tk-test-tracker-key-12345678901234567890",  # Valid format
            tracker_endpoint="https://api.cmdrdata.ai/api/events"
        )
...
try:
    ...
    from cmdrdata_openai.context import get_customer_context
    ...
    # Clear context
    from cmdrdata_openai import clear_customer_context
...
try:
    with patch('cmdrdata_openai.client.OpenAI') as MockOpenAI, \
        ...
        mock_openai = Mock()
        mock_openai.test_attribute = "test_value"
        mock_openai.test_method = Mock(return_value="method_result")
        MockOpenAI.return_value = mock_openai
        MockValidate.return_value = True

        client = TrackedOpenAI(
            api_key="test-key",
            tracker_key="tk-test-tracker-key-12345678901234567890"
        )
...
try:
    ...
    with patch('cmdrdata_openai.security.APIKeyManager.validate_api_key') as MockValidate:
        MockValidate.return_value = True

        tracker = UsageTracker(
            api_key="tk-test-tracker-key-12345678901234567890",
            endpoint="https://api.cmdrdata.ai/api/events",
            timeout=5.0
        )

        assert tracker.api_key == "tk-test-tracker-key-12345678901234567890", "Tracker API key not set"
...
try:
    import openai
    print(f"[INFO] OpenAI SDK version: {openai.__version__}")

    import cmdrdata_openai
    print(f"[INFO] CmdrData-OpenAI version: {cmdrdata_openai.__version__}")

    # Check if versions are compatible
    openai_version = tuple(map(int, openai.__version__.split('.')[:2]))
    min_version = (1, 0)

    if openai_version >= min_version:
        print(f"[OK] OpenAI version {openai.__version__} meets minimum requirement (>=1.0.0)")
    else:
        print(f"[WARNING] OpenAI version {openai.__version__} is below minimum (1.0.0)")
...
print("\n=== All Tests Completed ===")
print("Summary:")
print("- CmdrData-OpenAI imports successfully")
...
print(f"- Tracking methods configured: {len(OPENAI_TRACK_METHODS)} methods")
print(f"  * {', '.join(sorted(OPENAI_TRACK_METHODS.keys()))}")
